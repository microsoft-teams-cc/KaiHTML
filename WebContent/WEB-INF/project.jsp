	...
	<div class="col-md-2">
		<div class="panel panel-info">
			<ul class="list-group">
				...
				<li class = "list-group-item">
					<form method="post" onsubmit="uploadRevision()">
						<button type="submit" class="btn btn-link">Upload
							Revision</button>
					</form>
				</li>
			</ul>
		...
		function uploadRevision(){
			...
			$.each(projects, function(i, project_i) {
				if(project_i.replace(/\s/g, "") == project){
					if(!(owners[i] == sessionStorage.user)){
						alert("You cannot upload the revision. Project is owned by another owner");
					} else {
						alert("Upload the revised the project");
						statuses[i] = "Tobe Approved";
